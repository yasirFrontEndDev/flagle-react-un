...
import { GetDayNumber, GetNextDay } from './DailyRandom';
...
const Game = ({solution}) => {
    ...
    useEffect(() => {
        ...
        tempFlagToGuess.load(require(`../images/flags400/${solution.code}.png`), () => {
            solution.imageData = tempFlagToGuess.imageData.data
            solution.origImageData = tempFlagToGuess.imageData.data
        ...
        let storageState = JSON.parse(localStorage.getItem('flagle-state'))
    ...
    const loadGuesses = (pastGuesses) => {
        ...
        pastGuesses.forEach((guess, index) => {
            ...
            guessedFlag.load(require(`../images/flags400/${guess.code}.png`), () => {
                ...
                let percentage = guess.code === solution.code ? 100 : overlap === 1 ? 99.9 : overlap*100
                ...
                if(guessesToLoad.filter(g => g !== null).length === guessesToLoad.length) {
                    solution.solved = solution.code === guess.code ? 100.0 : (flagToDisplay.imageData.data.filter((p, i) => i % 4 === 3 && p).length/(solution.code === 'np' ? 33612 : 400*267)*100).toFixed(1)
    ...
    const displayPastGuesses = (guessesToLoad) => {
        ...
        if(animationsOn) {
            ...
            setTimeout(() => {
                ctx.drawImage(tempCanvas, 0, 0)
                if(guessesToLoad[guessesToLoad.length-1].code === solution.code) {
                    setGameOver(true)
                }
    ...
    const handleGuess = () => {
        if(selectedCountry !== null && guesses.filter(g => g.code === selectedCountry.code).length === 0) {
            if (guesses.length === 0) {
                let oldState = JSON.parse(localStorage.getItem('flagle-state'))
                if(oldState !== null && (oldState.win === false || oldState.dayNumber < theme.dayNumber-1)) {
                    let newStats = JSON.parse(localStorage.getItem('flagle-statistics'))
                ...
                setState(newState)
                localStorage.setItem('flagle-state', JSON.stringify(newState))
            }
            else {
                state.guesses.push(selectedCountry)
                localStorage.setItem('flagle-state', JSON.stringify(state))
            }
    ...
    const handleGameWin = () => {
        if (animationsOn) {
            setTimeout(() => {
                ...
                let oldState = JSON.parse(localStorage.getItem('flagle-state'))
                oldState.win = true
                localStorage.setItem('flagle-state', JSON.stringify(oldState))
            }, animationLength)
        }
        else {
            setAlertMessage("Nice one!")
    ...
    const updateStats = (win) => {
        let stats = JSON.parse(localStorage.getItem('flagle-statistics')) || {
            guesses: {
                1: 0,
                2: 0,
                3: 0,
        ...
        localStorage.setItem('flagle-statistics', JSON.stringify(stats))
    }
    ...
    function imageLoaded(guessedFlag){
        ...
        let overlap = matchingPixels/(400*267)
        let percentage =selectedCountry.code === solution.code ? 100 : overlap === 1 ? 99.9 : overlap*100
        ...
        solution.solved =(flagToDisplay.imageData.data.filter((p, i) => i % 4 === 3 && p).length/(solution.code === 'np' ? 33612 : 400*267)*100).toFixed(1)
        ...
        if(animationsOn) {
            setTimeout(() => {
                ...
                if(selectedCountry.code === solution.code) {
                    handleGameWin()
                }
    ...
    return (
        <div id="game" style={{width: canvasWidth+2}}>
            ...
            <GuessList guesses={guesses} resultColors={resultColors} setResultColors={setResultColors} showOriginal={showOriginal}></GuessList>
            {!gameOver && (
            ...
            {gameOver && (
                <div style={{marginTop: size === 1 || size === 5 || size === 9  ? "15px" :
                    ...
                    {gameLost && (
                        <div>
                            <GuessBox guess={solution}></GuessBox>
                        </div>
                    )}
                    <div >
                        <span style={{display: 'inline-block',
                                      ...
                                      }} onMouseOver={() => setTimerHover(true)} onMouseLeave={() => setTimerHover(false)}>
                        { timerHover && (
                            <span>Next Flagle</span>
                        )}
                        { !timerHover && (
                            <Countdown date={GetNextDay()} daysInHours/>
                        )}
                        </span>
                        <Button
                            id="share-btn"
